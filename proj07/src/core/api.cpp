...
namespace rt3 {
    ...
    void API::scene(
        ...
        for (auto ps : ps_objects) {
            ...
            else if (t.compare(ParserTags::OBJECT_TYPE_TRIANGLEMESH) == 0) {
                std::vector<std::shared_ptr<Shape>> triangles = create_triangle_mesh_shape(false, *(ps));
                std::shared_ptr<Material> m = materials[name];

                for (auto triangle : triangles) {
                    GeometricPrimitive geo(m, std::move(triangle));
                    std::shared_ptr<Primitive> geo_prim = std::make_shared<GeometricPrimitive>(geo);
